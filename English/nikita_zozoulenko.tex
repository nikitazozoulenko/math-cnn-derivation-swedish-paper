...
\begin{figure}[h]
	\centering
  	...
  	\caption{(a) The proposed convolutional-neural-network-based model FaceNet is capable of dynamically detecting over 700 faces in a crowded scene for various scales, lighting and occlusions. (b) A temporal neural network (TCN) and a Gated Recurrent Unit (GRU) automatically captioning an image..} \label{crowd}
\end{figure}
\vfill
...
Convolutional neural networks (CNNs) are traditionally used in computer vision tasks, but have recently gained traction as sequential models. I suggest two improvements to temporal convolutional networks (TCNs) and empirically evaluate them to achieve a new state of the art on the task of SequenceMNIST (99.53\% versus 99.0\%) and Permuted SequenceMNIST (97.86\% versus 97.28\%), using my new zero-padding and dilation scheme which also keeps information from flowing from the future into the past. Three different residual blocks are also experimented on to show how residual connections affect the performance of TCNs. The improved model is then compared to its recurrent neural network counterpart, a Gated Recurrent Unit (GRU), on an area where TCNs have never been applied before; automatic image captioning, and is shown to have similar performances. Additionally, since the field lacks a fully derived order 4 example of a CNN, I present a derivation of the general case for a CNN with an arbitrary input of a tensor of order 4, including varying zero-padding and strides during forward and backpropagation. The derivation is then used for my own neural network library implementation in Python and C++. Furthermore, I systematically construct a region based dense face detector and empirically investigate how factors such as cross entropy loss, focal loss, feature pyramid networks, online hard example mining, color jitter, and network depth affects its performance. The final model is sufficiently efficient and accurate to be used for real time video, for instance CCTV or other surveillance systems.
...
\section{Evaluating the models and handwritten digits}
The code and the trained models are publically available on github in the repository \textit{FCC-vs-CNN-mnist}.
...
If not otherwise stated, FaceNet is trained for 70 000 iterations on an Nvidia GTX 1080ti with an initial learning rate of 0.0001. Due to memory limitations a batch size of 8 is used. The network was trained on square image crops, randomly picked to be of between 0.3 and 1 of the short size of the image, and resized to be of size $512^2$ pixels. This was done to artificially increase the size of the training data. Additionally, the image was flipped horizontally with a probability of 0.5.

When evaluated on the WIDERFACE validation set, the model achieved a 0 mAP \cite{cs231n} across all 3 sets: easy, medium and hard. The model converged to classify every single bounding box as a negative example, resulting in no faces being found for every image.

\subsubsection{FaceNet v1.1, binary cross entropy}
The baseline model uses a softmax function based on two values, a scalar value of there exisiting a face and a scalar value of there not existing a face, to base the prediction on. An open source implementation of YOLO \cite{sigmoidvssoftmax} showed that using a binary cross entropy loss function (logistic regression), instead of a categorical cross entropy using a softmax function, increased the models accuracy. The softmax layer in FaceNet is replaced by a sigmoid function, and logistic regression is used instead. Let the new classification loss be denoted by $L_c$. Instead of normalizing the loss by the total number of bounding boxes, the loss is divided by the total number of positively assigned bounding boxes, similar to SSD \cite{ssd} and RetinaNet \cite{retinanet}.
...
When evaluated on the easy/medium/hard splits of the WIDERFace validation set, FaceNet v1.1 achieved a mAP of xx/yy/zz.

\subsubsection{FaceNet v1.2, focal loss}
...
Using the focal loss instead of binary cross entropy led to a decrease in mAP by approximately 10 points in every category. The results of the easy/medium/hard splits were 22.4	/12.1/5.1.

\subsubsection{FaceNet v1.3, feature pyramid}
...
Adding an FPN to FaceNet yielded a dramatical increase in mAP: The results on the easy/medium/hard splits were 89.3/85.9/65.9, a 4000\% inscrease in mAP.

\subsubsection{FaceNet v1.4, feature pyramid, focal loss}
...
The results on the easy/medium/hard splits were 81.0/81.0/64.2 mAP. Focal loss performed worse than logistic regression, when all other factors and hyperparameters were kept the same.

\subsubsection{FaceNet v1.5, feature pyramid, CE, level}
The loss function was modified to study the effects it had on the accuracy. Instead of normalizing all the losses by the total number of positively assigned anchors, each classification and regression loss is computed separately at every pyramid level. Let $P$ be the set of all pyramid levels $p$, $N^p$ be the set of all created anchor boxes at every spatial position at pyramid level $p$, and $N^p_{pos}$ be the set of all positively assigned anchors at pyramid level $p$. The total loss $L(\theta)$ is defined by:
...
\subsubsection{FaceNet v1.6, feature pyramid, focal loss, level}
...
The results on the easy/medium/hard set were 87.2/82.5/50.4 mAP.

\subsubsection{FaceNet v1.7, online hard example mining}
Online hard example mining (OHEM) \cite{ohem} has been shown to increase mAP when training region based object detectors. 
