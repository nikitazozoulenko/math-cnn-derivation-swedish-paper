...
Applying the OHEM algorithm on the whole mini-batch, instead of applying it on every image separately, yielded faster training times and higher accuracies. A $k$ value of 128 was used. The results on the easy/medium/hard set were xx/yy/zz mAP.

\subsection{Discussion}
benchmark speed

uses: cctv, surveilance, social media tagging, digital cameras.

focal loss is not better
...
\section{Conclusion} 
A mathematical derivation of the general case of a convolutional neural network with an arbitrary input of a tensor of order 4 is presented. Furthermore, the accuracy of temporal convolutional networks could be increased by using a linear dilation increase, together with more efficient zero-padding, to beat the current state of the art, with three different residual building blocks. Temporal convolutional networks were also shown to be a contender to its recurrent variant, the Gated Recurrent Unit, on the task of automatic image captioning, performing only marginally worse, and has the potential to perform better with additional changes. For dense face detection in images, focal loss and other types of loss functions were shown to perform worse than classical binary cross entropy. An addition of a feature pyramid network was shown to dramatically increase mean average precision. The final model is efficient and accurate enough to function as a real time face detector for surveillance and face tagging systems.
