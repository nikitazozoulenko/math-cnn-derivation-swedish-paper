...
\title{eller}
\title{Ansiktsigenkänning och Objektdetektering med Konvolutionella Neurala Nätverk}
...
För BN behöver det rekursiva delta-felet $\delta^{(l)}$, kostandsfunktionen med avseende på $\gamma_{c'}^{(l)}$ och kostandsfunktionen med avseende på $\beta{c'}^{(l)}$ beräknas. För att beräkna detta krävs något som heter kronecker-deltat, oftast betäcknat med $\delta_{i,j}$ men kommer nu vara betäcknat med $I_{i,j}$ på grund av att $\delta$ används för em annan term. Kronecker-deltat har följande egenskaper:
...
Först bryts $\inpd{L(W)}{X^{(l)}}$ upp i tre bråk och sedan summeras alla partiella derivator likt ekvation \eqref{konvolutionbackprop}:
...
$\inpd{L(W)}{X^{(l+1)}_{r,c,h,w}}$ är det föregående rekursiva delta-felet. $\inpd{X^{(l+1)}_{r',c',h',w'}}{\hat{X}_{r',c',h',w'}}$ hittas enkelt på grund av att den är en linjär funktion.
...
För derivatan av den centrerade aktiveringen med avseende på den originella aktiveringen tillämpas produktregeln:
...
Derivatan av den första faktorn med avseende på aktiveringen beräknas med hjälp av ekvationer \eqref{kroneckerdelta}, \eqref{kroneckerdeltaDERIVATIVE} och \eqref{kroneckerdeltaSUM}.
...
Derivatan av den andra faktorn med avseende på aktiveringen beräknas på ett liknande sätt med hjälp av kedjeregeln och ekvationer \eqref{kroneckerdelta}, \eqref{kroneckerdeltaDERIVATIVE} och \eqref{kroneckerdeltaSUM}.
...
Derivatan av vikterna hittas på ett liknande sätt med en faktor $\frac{1}{R}$ för att det är medelvärdet för alla exemepl i hopen som är eftersökt.
\begin{align}
\begin{split}
	\pd{L(W)}{\gamma^{(l)}_{c}}
...
En enkel CNN-modell kan användas för att klassificera handskriva siffror. För att uppnå detta har MNIST DATASET för handskriva siffror används. Den består av 60 000 unika exempel för träning och ytterligare 10 000 exempel för validering av modellen. Valideringsdatan måste vara separerad från datan man tränar med för annars finns det risk att modellen OVERFITTAS och inte kan generalisera för andra exempel än träningsdatan. 

Låt modellens prognos betäcknas med Softmaxfunktionen används för att gränsa output till intervallet $[0,1]$ och har egenskapen att alla prognostiserade värdena i ett exempel summeras till 1. Följaktligen kan varje värde i $y$ tolkas som sannolikheten att bilden är av varje klass.

\begin{figure}[h]\label{figMNIST}
	\centering
  		\includegraphics[scale=1]{mnist.png}
  	\caption{Tio bilder av handskrivna siffror från MNIST DATASETET}
\end{figure}

Input för modellen är en raw (rå??) $28 \times 28$ pixelarray. Modellen prognostiserar tio värden per bild i form av en tensor $\hat{y} \in \mathbb{R}^{R \times C}$, ett värde för varje klass $C=10$ av siffra. Om det näst sista lagret är $x$ med samma dimensioner som $\hat{y
}$ definieras softmaxfunktionen $P$ genom:
\begin{equation}
\begin{split}
P(x_{r,c}) 
	& = \hat{y}_{r,c} \\
...
\begin{split}
\pd{P(x_{r,c})}{x_{r,c}}
	& = \dfrac{(\sum^{C-1}_{c'=0}e^{x_{r,c'}}) (e^{x_{r,c}}) - (e^{x_{r,c}})(\sum^{C-1}_{c'=0}e^{x_{r,c'}} I_{c,c'})}{(\sum^{C-1}_{c'=0}e^{x_{r,c'}})^2} \\
	& = \dfrac{(e^{x_{r,c}})}{\sum^{C-1}_{c'=0}e^{x_{r,c'}}} - \dfrac{(e^{x_{r,c}})^2}{(\sum^{C-1}_{c'=0}e^{x_{r,c'}})^2} \\
	& = \hat{y}_{r,c}(1-\hat{y}_{r,c})
\end{split}
\end{equation}

Tillsammans med softmax används kostnadsfunktionen cross-entropy betäcknat med $L$ där $y$ är THE GROUND TRUTH:
...
\begin{figure}[h]\label{figepokmnist}
	\centering
  	...
  	\caption{En graf av validationsprecisionen efter varje epok av 60 000 bilder.}
\end{figure}
